...
Use this command when the user asks to install, refresh, update, or set up Volcano in Claude Code.

This plugin already ships the canonical Volcano skills, so this command only ensures the `volcano` CLI exists and is current. It does **not** download skills into `~/.volcano/skills`.

Run this idempotent shell block:
